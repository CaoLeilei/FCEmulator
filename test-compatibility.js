// 兼容性测试 - 测试经典ROM和场景
import { CPU } from './dist/core/cpu/index.js';
import { TestCartridge } from './dist/test-cartridge.js';
...
function testCompatibility() {
  ...
  runTest('循环计数经典模式', () => {
    ...
    memory.writeByte(0x8004, 0xFE); // -2 (回到0x8002)
  ...
  runTest('中断处理兼容性测试', () => {
    ...
    assertEqual(memory.readByte(0x01FC), 0x55);
  ...
  runTest('移位操作边界测试', () => {
    ...
    // 8次右移，应该回到原始值
    for (let i = 0; i < 8; i++) {
      memory.writeByte(0x8000 + i * 2, 0x6A); // ROR A
    }
  ...
  runTest('Nestest风格的基础验证', () => {
    ...
    for (let i = 0; i < tests.length; i++) {
      ...
      const addr = test[0];
      const opcode = test[1];
      const operand = test[2];
      const expectedA = test[3];
      const expectedFlags = test[4];
